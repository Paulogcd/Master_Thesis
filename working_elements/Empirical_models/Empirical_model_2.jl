...
# ╔═╡ 9c4adc6c-c618-4f8a-be93-a08bc2d0cc69
begin 
	using Plots
	using CSV
	using DataFrames
...
# ╔═╡ 4b67200c-2d75-11f0-14d3-5faf356cb331
md""" # Empirical model 2 """

# ╔═╡ 0a0e89be-066b-4017-9720-a419e85223c5
md""" # Setting 

This part is dedicated to the setting of the model. 

## Health transition estimates
...
begin 
	...
	df[:,:Health] 	= convert.(Float64,df[:,:Health])
	...
	# By two surveys: 
	df_2020_2022 	= leftjoin(df_2022,df_2020, on = :ID, makeunique=true)
	df_2018_2020 	= leftjoin(df_2020,df_2018, on = :ID, makeunique=true)
	df_2016_2018 	= leftjoin(df_2018,df_2016, on = :ID, makeunique=true)
	df_2014_2016 	= leftjoin(df_2016,df_2014, on = :ID, makeunique=true)
	df_2012_2014 	= leftjoin(df_2014,df_2012, on = :ID, makeunique=true)
	df_2010_2012 	= leftjoin(df_2012,df_2010, on = :ID, makeunique=true)
	df_2008_2010 	= leftjoin(df_2010,df_2008, on = :ID, makeunique=true)
	df_2006_2008 	= leftjoin(df_2008,df_2006, on = :ID, makeunique=true)
	df_2004_2006 	= leftjoin(df_2006,df_2004, on = :ID, makeunique=true)
	df_2002_2004 	= leftjoin(df_2004,df_2002, on = :ID, makeunique=true)

	df_2 			= vcat(df_2020_2022,
				df_2018_2020,
				df_2016_2018,
				df_2014_2016,
				df_2012_2014,
				...
				df_2004_2006,
				df_2002_2004)
	df_2 			= dropmissing!(df_2)

	# By three surveys: 
	df_2018_2022 	= leftjoin(df_2020_2022,df_2018, on = :ID, makeunique=true)
	df_2016_2020 	= leftjoin(df_2018_2020,df_2016, on = :ID, makeunique=true)
	df_2014_2018 	= leftjoin(df_2016_2018,df_2014, on = :ID, makeunique=true)
	df_2012_2016 	= leftjoin(df_2014_2016, df_2012, on = :ID, makeunique=true)
...
begin 
	df_3_v1 = copy(df_3)
	df_3_v1.Health = ifelse.(df_3_v1.Health .== 6, df_3_v1.Health_1, df_3_v1.Health)
	nothing
end

# ╔═╡ fc2c3fb0-50df-49c2-933b-28e1ddf405d9
md"""Using GLM to run a logistic regression to explain the status (binary variable) yields: """
...
# ╔═╡ 58adefd6-7cb1-4c08-91f8-7833faf0e47d
md"""Using a Ordinal Multinomial model to explain health (between 1 and 6) with the previous health status, we obtain:"""
...
md""" Using the MLJ package, we do not obtain a satisfactory result: """

# ╔═╡ ba11230b-67c6-4394-9904-c25dc27162a8
begin 
	# Formatting: 
	df_3_v2 = copy(df_3)
    df_3_v2.Health            	= categorical(df_3_v2.Health)
    df_3_v2.Health_1          	= categorical(df_3_v2.Health_1)
    df_3_v2.Health_2          	= categorical(df_3_v2.Health_2)
    df_3_v2.av_annual_t         = Float64.(df_3_v2.av_annual_t)
    df_3_v2.Age               = Float64.(df_3_v2.Age)
    y                       	= coerce(df_3_v2.Health, Multiclass)
    X                       	= select(df_3_v2, 
										 [:Health_1, :Health_2, :av_annual_t, :Age])
	...
	av_annual_range = 
		range(minimum(df_3_v2.av_annual_t)-1, maximum(df_3_v2.av_annual_t)+1, length=100)

    # 2. Get all Health_1 categories
    health_t_1_categories = levels(df_3_v2.Health_1)
    health_t_categories = levels(df_3_v2.Health)

    # Here, we need an age range: 
    age_range = range(minimum(df_3_v2.Age), maximum(df_3_v2.Age))
...
md""" We now want to visualise the predictions"""
...
begin 
    ...
    Poor        =
		DataFrame(Age = age_range,
				  Health_1 = fill(5,length(age_range)),
				  Health_2 = fill(5,length(age_range)),
    ...
    Fair        = 
		DataFrame(Age = age_range,
				  Health_1 = fill(4,length(age_range)),
				  Health_2 = fill(4,length(age_range)),
				  av_annual_t = fill(temperature_2018,length(age_range)),
				  GDP = fill(GDP_2018,length(age_range)),
				  Year = fill(2018,length(age_range)))
	
    Good        = DataFrame(Age = age_range,
							Health_1 = fill(3,length(age_range)),
							Health_2 = fill(3,length(age_range)),
							av_annual_t = fill(temperature_2018,length(age_range)),
							GDP = GDP_2018,
							Year = 2018)
	
    VeryGood    = DataFrame(Age = age_range,
							Health_1 = fill(2,length(age_range)),
							Health_2 = fill(2,length(age_range)),
							av_annual_t = fill(temperature_2018,length(age_range)),
							GDP = GDP_2018,
						   Year = 2018)
	
    Excellent   = DataFrame(Age = age_range,
	 						Health_1 = fill(1,length(age_range)),
	 						Health_2 = fill(1,length(age_range)),
	 						av_annual_t = fill(temperature_2018,length(age_range)),
	 						GDP = GDP_2018,
	 					   Year = 2018)

	pv1  = GLM.predict(model_1,Poor)
    fv1  = GLM.predict(model_1,Fair)
    gv1  = GLM.predict(model_1,Good)
    vgv1 = GLM.predict(model_1,VeryGood)
    ev1  = GLM.predict(model_1,Excellent)

    Plots.plot(pv1, label = "Poor - 1")
    Plots.plot!(fv1, label = "Fair - 1")
    Plots.plot!(gv1, label = "Good - 1")
    Plots.plot!(vgv1, label = "Very good - 1")
    Plots.plot!(ev1, label = "Excellent - 1")
...
MLJModels = "~0.17.9"
OrdinalMultinomialModels = "~0.4.5"
Plots = "~1.40.11"
PlutoUI = "~0.7.23"
"""
...
git-tree-sha1 = "1e385287972ab12b8fd46bf108c05f34cbf0be82"
...
[[deps.OrdinalMultinomialModels]]
deps = ["CategoricalArrays", "DelimitedFiles", "Distributions", "GLM", "Ipopt", "LinearAlgebra", "MathOptInterface", "NLopt", "RDatasets", "Random", "Reexport", "StatsBase", "StatsModels", "Tables"]
git-tree-sha1 = "58a3bd924396ad994a18a7680ae9389b168d5d77"
